...
class MyMovieServicesTest : ApiAbstract<MyMovieServices>() {
    ...
    /**
     * Test, to get list movies  from network and filled to movie response
     * */
    ...
    fun `get list movies from network and verify data`() = runBlocking {
        ...
        val response = service.getListMovies("movie", "now_playing", 1, null)
    ...
    /**
     * Test, to search movies from network and filled to movie response
     * */
    @Throws(IOException::class)
    @Test
    fun `search movie and verify data`() = runBlocking {
        enqueueResponse("/MovieResponse.json")
        val response = service.getListMovies("search", "movie", 1, "Mortal Kombat")
        val responseBody = requireNotNull(response.results)
        mockWebServer.takeRequest()
    ...
    /**
     * Test, to  get detail  from network and filled to movie
     * */
